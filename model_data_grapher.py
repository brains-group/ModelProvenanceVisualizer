...
def visualize_graph():
    ...
    for dataset_id, name in datasets.items():
        G.add_node(dataset_id, label=name, type="dataset")  # Explicitly setting the 'type' attribute

        # Add model nodes
    for model_id, name in models.items():
        G.add_node(model_id, label=name, type="model")  # Explicitly setting the 'type' attribute
